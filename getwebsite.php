...
//	require_once dirname(__FILE__) . '/PhpSpreadsheet/src/PhpSpreadsheet/Writer/IWriter.php';
//	require_once dirname(__FILE__) . '/PhpSpreadsheet/src/PhpSpreadsheet/IOFactory.php';
	// require_once dirname(__FILE__) . '/PhpSpreadsheet/src/PhpSpreadsheet/Spreadsheet.php';
    // require_once dirname(__FILE__) . '/PhpSpreadsheet/src/PhpSpreadsheet/Calculation/Category.php';
    // require_once dirname(__FILE__) . '/PhpSpreadsheet/src/PhpSpreadsheet/Calculation/Engine/CyclicReferenceStack.php';	
    // require_once dirname(__FILE__) . '/PhpSpreadsheet/src/PhpSpreadsheet/Calculation/Engine/Logger.php';
//    require_once dirname(__FILE__) . '/PhpSpreadsheet/src/PhpSpreadsheet/Calculation/Category.php';
//
//	
...
function getWebsite($idEvento){
	...
	$archijsdir=dirname(__FILE__). '/getwebsite.js';
	file_put_contents ($archijsdir,$archijscont);
	
	$phantom_script= dirname(__FILE__). '/getwebsite.js'; 
	...
	$div  = $doc->getElementById('live_left_column');
	$html = $doc->saveHtml($div);

	file_put_contents (dirname(__FILE__) .'/live_left_column.txt',$html);
...
/*
	...
	file_put_contents (dirname(__FILE__) .'/livematchtracker.txt',$html);
*/
				
	$Nequipos = preg_match_all("<a class=\"evento_menu selected\" evento_id=\"$idEvento\">",$html,$matches);
	
	echo "Nequipos es "."$Nequipos"."<br>";
	...
	*/
	...
	if ($div !=null){
		...
		if (file_exists ($filename)){
		  ...
		  $xls = IOFactory::load($filename);
				
		  $valor='dummy';
		  $columnaExcel=0;
		  while ($valor != ''){
			
			$colString = Coordinate::stringFromColumnIndex($columnaExcel);
